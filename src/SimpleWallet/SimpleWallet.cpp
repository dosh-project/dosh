...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    ...
    logger(INFO) << "IF the wallet seems to hang, just wait until syncing is done.";
...
bool simple_wallet::new_wallet(Crypto::SecretKey &secret_key, Crypto::SecretKey &view_key, const std::string &wallet_file, const std::string& password) {
  ...
  success_msg_writer() <<
    ...
    "current session's state. Otherwise, you will possibly need to synchronize \n" <<
    "your wallet again. Your wallet key is NOT under risk anyway.\n" <<
    "If shown -> Height <h> of <N> : Please wait for your wallet to sync when importing.\n" <<
    "**********************************************************************";
  return true;
}
//----------------------------------------------------------------------------------------------------
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    else
    {
      ...
      std::cout << "Windows:   simplewallet.exe --config-file configs/dosh.conf" << std::endl;
      std::cout << "Linux/Mac:   ./simplewallet --config-file configs/dosh.conf" << std::endl;
      return false;
    }
    po::notify(vm);
    return true;
