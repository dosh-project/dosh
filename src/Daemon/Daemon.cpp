...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    command_line::add_arg(desc_cmd_sett, arg_UPGRADE_HEIGHT_V5);
  ...
  command_line::add_arg(desc_cmd_sett, arg_genesis_block_reward_address);
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      else
      {
        ...
        std::cout << "Windows:   doshd.exe --config-file configs/dosh.conf" << std::endl;
        std::cout << "Linux/Mac:   ./doshd --config-file configs/dosh.conf" << std::endl;
        return false;
      }
      po::notify(vm);
...
{
  if (command_line::get_arg(vm, arg_CRYPTONOTE_NAME) == "dosh") {
